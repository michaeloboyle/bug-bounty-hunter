...
from api.main import (
    create_activity, create_activity_run, log_activity, 
    update_activity_status, add_artifact,
    ACTIVITIES, ACTIVITY_RUNS, ACTIVITY_LOGS, ARTIFACTS
)
...
class TestActivityIntegration:
    ...
    async def test_scan_creates_activity(self):
        ...
        from api.main import simulate_scan, SCAN_STATUSES
        ...
        assert activity["id"] in ARTIFACTS or any(aid in ARTIFACTS for aid in activity["artifacts"])
    ...
    def test_activity_filtering_logic(self):
        ...
        program1_activities = [a for a in ACTIVITIES if a.get("programId") == "program-1"]
        assert len(program1_activities) == 2
        
        # Test combined filtering
        program1_scans = [a for a in ACTIVITIES if a["type"] == "scan" and a.get("programId") == "program-1"]
